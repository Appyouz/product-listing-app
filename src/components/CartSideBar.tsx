...
const CartSidebar = () => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white dark:bg-card-dark shadow-xl z-50 transform transition-transform duration-300 
                    ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-4 flex justify-between items-center border-b dark:border-gray-700">
          <h2 className="text-xl font-bold dark:text-primary-text-dark flex items-center">
        ...
        <div className="p-4 h-full overflow-y-auto">
          {cartItems.length === 0 ? (
            <p className="text-center text-gray-500 dark:text-secondary-text-dark mt-10">
              Your cart is empty.
            </p>
          ) : (
            <>
              {cartItems.map((item, index) => (
                <div key={item.product.id} className="flex items-center mb-4 p-2 border-b dark:border-gray-800">
                  <img src={item.product.thumbnail} alt={item.product.title} className="w-12 h-12 object-cover rounded mr-3" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold dark:text-primary-text-dark truncate">{item.product.title}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Qty: {item.quantity}</p>
                    <p className="text-xs text-indigo-600 dark:text-gray-400">${(item.product.price * item.quantity).toFixed(2)}</p>
                  </div>
                </div>
              ))}
              <div className="absolute bottom-0 left-0 w-full p-4 border-t dark:border-gray-700 bg-white dark:bg-card-dark">
                <p className="text-lg font-bold mb-2 dark:text-primary-text-dark">Total: ${cartTotal.toFixed(2)}</p> {/* ⬅️ USE cartTotal */}
                <button
